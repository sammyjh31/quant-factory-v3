...
The scaffold exists so live LLM experiments can be measured, compared, and contained. Goal 11E comparison-note update is complete. The next proposed step is Goal 12A planning for a line-range-first locator contract.
...
2. `chunked_source_grounding`
   ...
   - Current state: active live-pilot lab with proposal-only export records, manual reviews, a bounded negative Flash result, a Pro narrowed-contract result, a second-source source-span precision repeat, strict source-span re-review records, and a local comparison note pointing to a line-range-first locator contract as the next planning fork.
...
Goal 11E comparison-note update is complete. The next proposed step is Goal 12A planning for a line-range-first locator contract.
...
The updated comparison note records that direct locator emission partly improves review because line ranges are useful, but char offsets are not yet accurate enough for canonical quote-hash support. Goal 12A should plan a line-range-first locator contract without creating validation, graduation, or architecture.
