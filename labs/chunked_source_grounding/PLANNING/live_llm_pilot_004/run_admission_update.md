...
Status: Goal 9A live-run admission update for completed proposal-only pilot 004
...
It did not by itself authorize execution. Execution required a separate Goal 9B instruction.

Goal 9A itself did not execute the run. The separately authorized run now has proposal-only export records for `chunked_source_grounding_live_pilot_004`.
