...
| Axis | Long-context pilot 001 | Chunked Flash pilot 001 | Chunked Pro pilot 002 |
| --- | --- | --- | --- |
| source grounding | Mostly specific span hints, but still affected by `missing_context`. | Not content-reviewable because truncation blocked source-grounding review. | Source-linked at a claim level; caveat: `broad_segment_refs` rather than canonical offsets. |
| research usefulness | Useful as a broader judgment-abstraction baseline. | Useful as negative-result value for output-contract sizing. | Useful for testing whether narrowed chunked contracts can produce reviewable source-grounded artifacts. |
| hallucination / unsupported claims | Separated promotional or unsupported claims reasonably, with caveats. | Not reviewable as a completed artifact. | Unsupported-claim report did not add unsupported material, but the zero-count report is not comprehensive proof. |
| abstraction quality | Stronger broader judgment abstraction, with `over_abstracted_teacher_intent` caveat. | No reliable abstraction result because the artifact failed structurally. | `limited_abstraction` is expected because the narrowed contract avoided broad judgment commentary. |
| negative-result value | Revealed missing-context and teacher-intent compression issues. | High negative-result value: the Flash contract was too broad for the configured cap. | Shows that narrowing the output contract can trade breadth for structural completion. |
| output-contract fit | Fit was acceptable for a compact long-context proposal. | Poor fit: output contract exceeded the admitted Flash output budget. | Better fit: smaller contract produced parseable reviewable output. |
| comparison value for future method design | Baseline for broader judgment extraction. | Warning that chunked/source-grounded prompts can fail from contract size before method quality is testable. | Candidate direction for tighter source-span precision tests. |
...
The tentative next step is to improve source-span precision for chunked Pro before claiming method quality. A later comparison can test whether canonical span IDs or tighter segment references improve grounding without re-expanding the output contract. A second long-context variant with stricter grounding may also be useful, but this note does not choose that path as architecture.
