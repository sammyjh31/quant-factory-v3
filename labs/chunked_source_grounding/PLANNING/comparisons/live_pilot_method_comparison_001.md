...
| Axis | Long-context pilot 001 | Chunked Flash pilot 001 | Chunked Pro pilot 002 | Chunked Pro pilot 003 |
| --- | --- | --- | --- | --- |
...
| source-span precision | Useful hints, but not designed as a span-precision contract. | Not reviewable as a completed artifact. | Broad segment refs made the artifact reviewable but imprecise. | Improved over pilot 002; still lacks canonical offsets. |
...
| negative-result value | Revealed missing-context and teacher-intent compression issues. | High negative-result value: the Flash contract was too broad for the configured cap. | Shows that narrowing the output contract can trade breadth for structural completion. | Shows the narrowed contract can be refined for tighter support labels while preserving parseability. |
| output-contract fit | Fit was acceptable for a compact long-context proposal. | Poor fit: output contract exceeded the admitted Flash output budget. | Better fit: smaller contract produced parseable reviewable output. | Good fit: source-span precision contract remained parseable and reviewable. |
| comparison value for future method design | Baseline for broader judgment extraction. | Warning that chunked/source-grounded prompts can fail from contract size before method quality is testable. | Baseline for complete chunked Pro source-grounding under a narrow contract. | Candidate method variant to repeat on a second source excerpt before broader claims. |
...
The leading next research direction is to repeat source-span precision on a second source excerpt before choosing another live-method fork.

This would test whether the pilot 003 refinement generalizes beyond the first excerpt and whether tighter source-span hints remain parseable, reviewable, and useful outside the original source slice.

Alternative later directions remain open: a stricter long-context grounding variant, a visual/deictic pilot, or evaluator calibration. This note does not choose any direction as architecture.
