...
The line-range-first locator pilot 006 tested the next narrowed locator contract. Its strict line-range review, `chunked_source_grounding_live_pilot_006_strict_line_range_review`, recorded score `0.88`, pass_fail `pass`, line-range candidates reviewed `3`, exact `2`, approximate `1`, broad `0`, missing `0`, overclaimed exactness `0`, local offsets computable `3/3`, and local quote hashes computable `3/3`. The key finding is that line-range-first improved over pilot 005 by avoiding model-proposed char offsets and letting local review compute offsets/hashes after validation.
...
Pilot 006 tested that bottleneck directly. The strict line-range review passed with score `0.88`: line-range candidates reviewed `3`, exact `2`, approximate `1`, broad `0`, missing `0`, overclaimed exactness `0`, local offsets computable `3/3`, and local quote hashes computable `3/3`. pilot 006 shows line-range-first locator candidates are a better model-facing contract than line-plus-character-offset locator emission.

The current method insight is narrow and useful: Ask the model for line ranges; let local review/tooling compute offsets and quote hashes after validation. Local offset/hash computation after line-range validation is the stronger workflow.
...
Goal 12A planned the line-range-first locator contract at `labs/chunked_source_grounding/PLANNING/line_range_locator_contract_001/`. The contract's useful responsibility split is: model emits line ranges and local review computes offsets/hashes only after line-range validation.

The line-range-first pilot supports a third narrow conclusion: local offset/hash computation after line-range validation is the stronger workflow. It avoids asking the model to emit brittle character offsets or cryptographic hashes, while still giving reviewers support-valid regions that can be turned into local offsets and hashes.

Recommend pausing further locator-contract refinement until the portfolio chooses the next bounded fork:

1. repeat line-range-first on a third source excerpt;
2. compare against a stricter grounded long-context variant;
