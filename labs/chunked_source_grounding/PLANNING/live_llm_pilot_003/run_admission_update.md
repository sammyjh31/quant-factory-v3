# Chunked Source Grounding Live Pilot 003 Run Admission Update

Status: Goal 8B live-run admission update for completed proposal-only pilot 003
...
It did not by itself authorize execution. Execution required a separate Goal 8C instruction.

Goal 8B itself did not execute the run. The separately authorized run now has proposal-only export records for `chunked_source_grounding_live_pilot_003`.
...
This update narrows the Goal 8B planning packet into one executable preflight scope for Goal 8C if separately authorized.
...
The Pro/narrow-contract pilot 002 record remains the current completed chunked Pro run and is not overwritten by this source-span precision variant.
...
Purpose: test whether a source-span precision contract can tighten support hints beyond broad segment references while staying complete, parseable, and bounded.
...
This pilot tests a deliberate source-span precision variant, not a replacement for `chunked_source_grounding_live_pilot_002`.
...
Prompt template SHA-256: `09555de445821f26baf834a7cb2fdb32a95ef7514302fea028c13de81fc7c177`
...
The max output cap remains intentionally small; this run tests whether tighter source-span support hints can fit inside the tiny pilot boundary.
...
```text
provider_payloads/chunked_source_grounding_live_pilot_003/
model_traces/chunked_source_grounding_live_pilot_003/
prompt_traces/chunked_source_grounding_live_pilot_003/
```
