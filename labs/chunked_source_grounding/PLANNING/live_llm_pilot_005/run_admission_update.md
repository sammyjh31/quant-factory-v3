...
This update narrows the Goal 11B planning packet into one executable preflight scope for Goal 11C if separately authorized.
...
The pilot 004 source-span precision record remains the current completed second-source source-span precision run and is not overwritten by this locator-candidate pilot.
...
Purpose: test whether a source-span locator candidate contract can make the model emit canonical line ranges and character offsets directly, so local review can compute quote hashes without reconstructing support from broad segment hints.
...
This pilot tests a deliberate locator-candidate variant, not a replacement for `chunked_source_grounding_live_pilot_004`.
...
```text
same approved source as pilot 004: operator-approved second-source excerpt from the local pharm box-trades transcript
```
...
The source text must stay local-only or ignored. The committed record may include source scope identifiers, line ranges, character offsets, locator labels, locator confidence, computed quote hashes, and metadata-safe summaries, but not raw private source text unless a later explicit policy allows it.
...
Prompt template SHA-256: `74ad3e97e88e0dc9782f50668fc9e0669fdc95443837532f67d10d05d074a1d3`
...
The output contract is the source-span locator candidate contract from Goal 11A.
...
Local runner/reviewer computes quote hashes after the model response from selected local source spans.
...
The max output cap remains intentionally small; this run tests whether the locator-candidate contract fits without expanding into broader judgment abstraction.
...
```text
provider_payloads/chunked_source_grounding_live_pilot_005/
model_traces/chunked_source_grounding_live_pilot_005/
prompt_traces/chunked_source_grounding_live_pilot_005/
```
...
```text
labs/chunked_source_grounding/EXPORTS/run_record.live_pilot_005.json
labs/chunked_source_grounding/EXPORTS/artifact_envelope.live_pilot_005.json
labs/chunked_source_grounding/EXPORTS/evaluation_record.live_pilot_005.json
labs/chunked_source_grounding/EXPORTS/research_note.live_pilot_005.json
```
