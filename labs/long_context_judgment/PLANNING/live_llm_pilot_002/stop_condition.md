...
Goal 13C, if separately authorized, must stop before any model call if:
...
* the output contract expands beyond the five approved output sections,
* the prompt asks the model to emit character offsets,
* the prompt asks the model to emit quote hashes,
* judgment principle proposals cannot be evaluated against their cited supporting claims,
